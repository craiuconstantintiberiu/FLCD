...
public class SymbolTable {
    ...
    public List<Pair<String, Integer>> getAllElements() {
        List<Pair<String, Integer>> allElements = new LinkedList<>();
        for (int pos = 0; pos < elements.length; pos++) {
            if (elements[pos] != null) {
                allElements.add(new Pair<>(elements[pos], pos));
            }
        }
        return allElements;
    }
    ...
    private int findFirstEmptyPosition(int start) {
        ...
        for (int pos = start; pos < capacity && pos>=0; pos++) {
            if (isPositionEmpty(pos)) {
                return pos;
            }
        }
    ...
    private int computeHash(String toBeAdded) {
        int hash = toBeAdded.hashCode()>0?toBeAdded.hashCode() : -toBeAdded.hashCode();
